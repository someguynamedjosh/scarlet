...
pub(super) fn create_from_dex(
    env: &Environment,
    from: ItemPtr,
) -> ItemPtr {
    let scope = || SPlain(from);
    let into = Item::placeholder_with_scope(Box::new(scope()));
    from.borrow_mut().from_dex = Some(into);
    ...
    if let Some(structt) = from.downcast_definition::<DPopulatedStruct>() {
        ...
        redefine_as_and(env, into, first_two, rest_from_rest);
        ...
        let statement = if invs.len() == 0 {
            ...
            for &part in &invs[1..] {
                statement = create_and(env, statement, part, scope());
            }
            statement
        };
        ...
        let equal = DDecision::new(x.ptr_clone(), from, truee.ptr_clone(), falsee.ptr_clone());
