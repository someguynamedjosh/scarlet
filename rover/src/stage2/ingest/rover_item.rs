...
fn define_binary_op(
    ...
    let base = env.insert(Item::PrimitiveOperation(op(a, b)));
...
fn define_integer_type(
    ...
    let itype_base = env.insert(Item::PrimitiveType(typee));
...
fn define_bool_type(env: &mut Environment) -> ItemId {
    let bool_type_base = env.insert(Item::PrimitiveType(PrimitiveType::Bool));
    let true_con = env.insert(Item::PrimitiveValue(PrimitiveValue::Bool(true)));
    let false_con = env.insert(Item::PrimitiveValue(PrimitiveValue::Bool(false)));
...
fn define_lang_item(env: &mut Environment) -> (ItemId, ItemId) {
    let god_type = env.insert(Item::GodType);

    let lang_members = define_lang_members(env, god_type);
    let lang_item = env.insert(Item::defining(god_type, lang_members));
...
pub fn define_rover_item(env: &mut Environment) -> (ItemId, ItemId) {
    ...
    let rover = env.insert(Item::defining(god_type, rover_members));
